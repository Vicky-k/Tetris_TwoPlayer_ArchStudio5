...
public class BoardPanelTwo extends JPanel{
	/**
	 * Serial Version UID.
	 */
	int[] addlines = {100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100} ;
	...
	public BoardPanelTwo(TetrisImp tetris) {
		this.tetris = tetris;
	 ...
	 */
	public int checkLines(BoardPanelOne boardOne) {
	 ...
	 */
	public boolean checkLine(int line, BoardPanelOne boardOne) {
		/*
		...
		boardOne.otherPlayer(this, line);
	...
	public void paintComponent(Graphics g) {
		...
		if(tetris.isPaused2()) {
		...
		} else if(tetris.isNewGame2() || tetris.isGameOver2()) {
			g.setFont(LARGE_FONT);
			g.setColor(Color.WHITE);
			 ...
			 */
			String msg = tetris.isNewGame2() ? "TETRIS" : "GAME OVER";
			g.drawString(msg, CENTER_X - g.getFontMetrics().stringWidth(msg) / 2, 150);
			g.setFont(SMALL_FONT);
			msg = "Press Alt to Play" + (tetris.isNewGame2() ? "" : " Again");
			 ...
			 */
			TileType type = tetris.getPieceType2();
			int pieceCol = tetris.getPieceCol2();
			int pieceRow = tetris.getPieceRow2();
			int rotation = tetris.getPieceRotation2();
			if(type == null)
					System.out.println("problem with enum");
			else
	...
	public void otherPlayer(BoardPanelOne boardOne, int line){
		...
		addlines[addlinesindex]=peak;
		addlinesindex++;
		for(int col = 0; col < COL_COUNT; col++){
		setTile(col, peak, boardOne.getTile(col, line));
	}
