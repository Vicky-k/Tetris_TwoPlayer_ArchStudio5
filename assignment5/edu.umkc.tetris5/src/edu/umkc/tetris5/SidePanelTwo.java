...
public class SidePanelTwo extends JPanel{
	/**
	 * Serial Version UID.
	 */
	 ...
	 */
	public static final int TILE_SIZE = BoardPanelTwo.TILE_SIZE >> 1;
	
	/**
	 * The width of the shading on each tile on the next piece preview.
	 */
	public static final int SHADE_WIDTH = BoardPanelTwo.SHADE_WIDTH >> 1;
	 ...
	 */
	public SidePanelTwo(TetrisImp tetris) {
		this.tetris = tetris;
		
		setPreferredSize(new Dimension(200, BoardPanelTwo.PANEL_HEIGHT));
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString("PLAYER2 GAME",SMALL_INSET, offset = STATS_INSET-30);
		g.drawString("Stats", SMALL_INSET, offset = STATS_INSET);
		g.setFont(SMALL_FONT);
		g.drawString("Level: " + tetris.getLevel2(), LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Score: " + tetris.getScore2(), LARGE_INSET, offset += TEXT_STRIDE);
		...
		g.setFont(SMALL_FONT);
		g.drawString("X - Move Left", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("C - Move Right", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("V - Rotate Anticlockwise", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("B - Rotate Clockwise", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("Z - Drop", LARGE_INSET, offset += TEXT_STRIDE);
		g.drawString("N - Pause Game", LARGE_INSET, offset += TEXT_STRIDE);
		 ...
		 */
		TileType type = tetris.getNextPieceType2();
		if(!tetris.isGameOver2() && type != null) {
			/*
